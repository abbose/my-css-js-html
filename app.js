...
function newsock()
{
   ...
   socket.on('connect', function () {
    lstat('success', 'اتصل'); $('#tlogins button').removeAttr('disabled');
...
function AddMsg(wid, data) {
  ...
  if (data.bid != null) {
    ...
    msg.append('<a onclick="send(\'likebc\',{bid:\'' + data.bid + '\'})" style="margin-top:-20px;padding:4px;border-radius:15px 15px 15px 0px;background-color:#990033;" class="btn minix btn-danger fa fa-heart fr">&nbsp;</a>')
  }
  if (isbc == true) {
    msg.prependTo($(wid))
  }
...
function close(i)
{ if (isclose) { return; } isclose = true; window.onbeforeunload = null; setTimeout('location.reload();', i || 4000); lstat('info', 'انتظر قليلآ') }
...
function upro(id) {
  ...
  ht.find('.uh').css('background-color', "").off().click(function () {
    ...
    popdiv(div, 'معلومات الشخص');
    $.get("uh?token=" + token + "&u2=" + id, function (d) {
      if (typeof d == 'object') {
        $.each(d, function (i, e) {
          var dd = $("<div class='borderg'></div>");
          dd.append('<label class="label label-info">عضويته</lable><br>');
...
function mkr() {
  ...
  ht.find('.modal-title').text('روم جديد, حياه جديده ^_^');
...
function redit(id) {
  ...
  if (r == null) { return; }
  var ht = $("#mkr");
  ht.find('.modal-title').text('تعديل الروم');
